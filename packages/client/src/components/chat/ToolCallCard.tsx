...
import { getToolConfig, getToolSummary, isErrorResult, getLanguageFromPath } from '../../lib/toolFormatters';
...
function EditDetail({ input }: { input: Record<string, any> }) {
  return (
    <div className="px-3 py-2 space-y-2">
      <p className="text-xs font-mono text-slate-400 truncate">{input.file_path}</p>
      {input.old_string && (
  ...
  return (
    <div className="px-3 py-2 space-y-1">
      <p className="text-sm text-slate-200">
        ...
        {input.path && (
          <span className="text-slate-400 ml-2 text-xs">in {input.path}</span>
        )}
      </p>
...
function TaskDetail({ input }: { input: Record<string, any> }) {
  return (
    <div className="px-3 py-2 space-y-1">
      ...
      {input.description && (
        <p className="text-sm text-slate-300">{input.description}</p>
      )}
      {input.prompt && (
        <p className="text-xs text-slate-400 line-clamp-3">{input.prompt}</p>
      )}
    </div>
  );
}
...
function WebFetchDetail({ input }: { input: Record<string, any> }) {
  return (
    <div className="px-3 py-2 space-y-1">
      {input.url && (
        <p className="text-sm font-mono text-teal-300 truncate">{input.url}</p>
      )}
      {input.prompt && (
        <p className="text-xs text-slate-400">{input.prompt}</p>
      )}
    </div>
  );
}
...
function ToolResultInline({
  ...
  return (
    ...
    >
      ...
      <pre
        className={`text-sm font-mono text-slate-300 overflow-x-auto whitespace-pre-wrap ${
          resultExpanded ? 'max-h-96' : 'max-h-32'
        } overflow-y-auto`}
      >
